const getGamingVideos = async (result:any) => {
    try {
      ...
      const videoResponse = await fetch(
        `https://www.googleapis.com/youtube/v3/search?key=${process.env.API_KEY}&q=gaming&part=snippet&type=video&order=date&maxResults=${result}&publishedAfter=${oneWeekAgo.toISOString()}`
      );
  ...
  export default getGamingVideos;
