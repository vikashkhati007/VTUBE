const GetSuggestVideos = async () => {
  try {
    ...
    const videoResponse = await fetch(
      `https://www.googleapis.com/youtube/v3/search?key=${process.env.API_KEY}&part=snippet&type=video&order=date&maxResults=10&publishedAfter=${oneWeekAgo.toISOString()}`
    );
...
export default GetSuggestVideos;
